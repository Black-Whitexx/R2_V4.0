...
void locater_Data_Rec(uint8_t *data, locater_def *loc)
{
    ...
    if(data[0] == 0xBB && data[1] == 0xCC)
    {
        ...
        loc->pos_x = Union_loc.data_f[0] + loc->pos_x_base - 0.9* arm_cos_f32(loc->angle * 3.1415926 / 180) + 0.9;
        loc->pos_y = Union_loc.data_f[1] + loc->pos_y_base - 0.9* arm_sin_f32(loc->angle * 3.1415926 / 180);

        loc->angle = Union_loc.data_f[2];

//        printf("%f,%f,%f\n",loc->pos_x,loc->pos_y,loc->angle);
